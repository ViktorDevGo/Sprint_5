...
func (t Training) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	p := InfoMessage{TrainingType: t.TrainingType, Duration: t.Duration, Distance: t.distance(), Speed: t.meanSpeed(), Calories: t.Calories()}
	return p
}
...
type CaloriesCalculator interface {
	...
	Calories() float64         //метод возвращает количество килокалорий, израсходованных во время тренировки
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	return ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Training.Weight / MInKm * float64(r.Training.Duration.Hours()) * MinInHours)
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	z := w.Training.meanSpeed() * KmHInMsec / w.Training.Duration.Hours()
	return ((CaloriesWeightMultiplier*w.Training.Weight + ((math.Pow(z, 2))/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Training.Weight) * float64(w.Training.Duration.Hours()) * MinInHours)
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	p := InfoMessage{TrainingType: s.Training.TrainingType, Duration: s.Training.Duration, Distance: s.Training.TrainingInfo().Distance, Speed: s.meanSpeed(), Calories: s.Calories()}
	return p
}
